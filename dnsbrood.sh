...
function doCache()
{
  case "$1" in
      stat)
        java -jar $HOME_DIR/lib/DNSQueen-1.0-SNAPSHOT.jar -cstat_cache
        ;;
      dump)
        echo "Dump cache to $HOME_DIR/cache.dump"
        java -jar $HOME_DIR/lib/DNSQueen-1.0-SNAPSHOT.jar -cdump_cache > /dev/null
        ;;
      clear)
        echo "Clear cache"
        java -jar $HOME_DIR/lib/DNSQueen-1.0-SNAPSHOT.jar -cclear_cache
        ;;
      *)
...
case "$1" in
  ...
  stop)
    ...
    java -jar $HOME_DIR/lib/DNSQueen-1.0-SNAPSHOT.jar -cshutdown > /dev/null
    ;;
  cache)
    doCache $2
    ;;
  ...
  reload)
    ...
    java -jar $HOME_DIR/lib/DNSQueen-1.0-SNAPSHOT.jar -creload > /dev/null
    ;;
  zones)
